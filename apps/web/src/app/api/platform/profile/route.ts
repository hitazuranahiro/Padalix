...
async function forward(method: "GET" | "PATCH", request?: Request) {
  ...
  try {
    ...
    return Response.json(await platformRequest(session, "/v1/profile", {
      method,
      ...(body ? { body: JSON.stringify(body) } : {}),
    }));
  } catch (error) {
    const status = error instanceof PlatformError ? error.status : 500;
    return Response.json({ error: error instanceof Error ? error.message : "Profile request failed." }, { status });
  }
}
