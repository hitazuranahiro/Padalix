...
export function OperationsConsole({ snapshot }: { snapshot: OperationsSnapshot }) {
  ...
  return <div className="operations-console">
    <section className="operations-summary">
      ...
      <div className="operations-metrics"><article><strong>{snapshot.transfers.length}</strong><span>RECENT TRANSFERS</span></article><article data-alert={problemJobs.length > 0}><strong>{problemJobs.length}</strong><span>FAILED JOBS</span></article><article data-alert={openExceptions.length > 0}><strong>{openExceptions.length}</strong><span>OPEN EXCEPTIONS</span></article></div>
