...
4. The browser uploads directly to private storage with signed content type, size, SHA-256, metadata, and server-side-encryption constraints.
5. Finalization performs `HeadObject` with checksum retrieval. Size, type, signed metadata, and the storage-computed checksum must all match before a KYC case is created.
...
- Require TLS and server-side encryption. Set `KYC_EVIDENCE_S3_KMS_KEY_ID` to require the configured KMS key; otherwise uploads use S3-managed AES-256 encryption.
