...
export function OperationsConsole({ snapshot }: { snapshot: OperationsSnapshot }) {
  ...
  return <div className="operations-console">
    <section className="operations-summary">
      ...
      <div className="operations-metrics"><article><strong>{snapshot.transfers.length}</strong><span>RECENT TRANSFERS</span></article><article data-alert={snapshot.queue.failed > 0}><strong>{snapshot.queue.failed}</strong><span>FAILED JOBS</span></article><article data-alert={openExceptions.length > 0}><strong>{openExceptions.length}</strong><span>OPEN EXCEPTIONS</span></article><article data-alert={!workerHealthy}><strong>{workerHealthy ? "LIVE" : "DOWN"}</strong><span>WORKER RUNTIME</span></article><article data-alert={snapshot.queue.oldestPendingSeconds > 60}><strong>{snapshot.queue.pending}</strong><span>PENDING / {snapshot.queue.oldestPendingSeconds}S OLDEST</span></article></div>
